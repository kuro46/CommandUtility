...
public abstract class RootCommand extends Command {
    ...
    private List<String> onTabComplete(
        ...
        final Arg argInfo = Optional.ofNullable(Iterables.get(argsInfo, free.size() - 1, null))
            .orElse(Iterables.getLast(argsInfo));
        final String currentValue = Optional.ofNullable(Iterables.getLast(free, null))
            .orElseGet(() -> Iterables.getLast(result.getUsed()));
        // Build data
